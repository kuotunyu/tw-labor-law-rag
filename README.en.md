...
A fail-closed US$5-per-provider runner now exists for Gemini `gemini-3.5-flash-lite` and OpenAI `gpt-5.6-luna`. Dedicated project credentials were not available locally, so the formal provider cross-check is explicitly **`pending_credentials`**; no unrelated `.env`, substitute model, or fabricated provider evidence was used.
...
This is the `v0.3.1` source-only runtime and deployment release. Its formal model-quality metrics retain the unchanged `v0.1.0` formal evidence baseline. This release adds a full-corpus snapshot, per-citation legal-source provenance, a 60-question reliability stress benchmark, and a hard-capped dual-provider runner. The formal Gemini/OpenAI cross-check is explicitly `pending_credentials` because no dedicated project keys were available locally; no unrelated key or substitute model was used. It is an evidence-backed software portfolio artifact, not legal advice and not a production legal service. The complete corpus, model weights, private indexes, and raw provider artifacts remain outside this source release.
