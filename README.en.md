...
This is the `v0.3.4` source-only runtime and deployment release. Its formal model-quality metrics retain the unchanged `v0.1.0` formal evidence baseline. On top of v0.3.3's new/old-regime severance hardening, this release adds deterministic Article 14 retrieval assistance for matching wage-arrears/immediate-exit questions without presenting that routing contract as a recomputed quality gain. The v0.3.2 Gemini/OpenAI safety cross-check completed against the fixed models with five requests each under the US$5 cap: Gemini observed refusal accuracy `0.8`, citation success `1.0`, and estimated cost `US$0.0022620`; OpenAI observed refusal accuracy `1.0`, citation success `1.0`, and estimated cost `US$0.0026414`. Its strict content-free public trace excludes question/answer text, provider payloads, and credentials, and the cross-check does not replace the formal model-quality baseline. It is an evidence-backed software portfolio artifact, not legal advice and not a production legal service. The complete corpus, model weights, private indexes, and raw provider artifacts remain outside this source release.
