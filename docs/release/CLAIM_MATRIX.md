...
| 壓力集 Hit@5=0.950、MRR@10=0.908、直接誤拒 1/40、攔截 17/20；formal guard 仍為 0.967/0.906、0/30、9/10;README、EVAL_REPORT | offline-recomputed | fixed model revisions、snapshot contract、`src/rag/config.py` | `eval/official/reliability_trace.jsonl`；`eval/official/reliability_formal_trace.jsonl` | `eval/official/reliability_results.json` | `tests/test_reliability.py`;release verifier | verifier 從兩份 trace 重算完整 sweep 與 Pareto 決策；0.03 無較佳候選，故保留設定；不代表誤拒已消失 |
| v0.3.5 compact portfolio regression：6/6 可答來源契約、10/10 路由與檢索階段決策契約、provider calls=0;EVAL_REPORT、official README | offline-recomputed | `eval/dataset/portfolio_demo_v0.3.5.jsonl`、pinned retrieval config、`release/corpus_snapshot.json` | result 中 10 筆 content-free cases | `eval/official/portfolio_demo_v0.3.5.json` | `tests/test_portfolio_demo_regression.py`、`tests/test_official_artifacts.py`、release verifier | 2 題 threshold refusal；2 題只證明正確到達 LLM boundary，artifact 沒有執行或評分 final LLM refusal；不取代 40/60 題 evidence |
| 2026-08-30 人工 freshness audit 確認 15 部／884 條逐條文指紋 current;README、release notes | official-source comparison + offline-verified artifact | `scripts/audit_corpus.py`、`release/corpus_snapshot.json` | `release/corpus_article_snapshot.json` 的條號與 SHA-256 | release verifier 的 `source_data.article_snapshot_*` | `tests/test_corpus_audit.py`、`tests/test_official_artifacts.py`、release verifier | 不發布法規全文／chapter／URL；只在操作人員在場時執行，沒有排程，也不表示 2026-08-30 後永遠未變 |
